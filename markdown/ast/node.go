...
// Emoji represents markdown emoji inline node
type Emoji struct {
	Leaf
}
...
// DocuowlBox represents markdown heading node
type DocuowlBox struct {
	Container
	Title        string
	IsDocuOwlBox bool
}

// DocuowlList represents markdown heading node
type DocuowlList struct {
	Container
	Title    string
	HasTitle bool
}

type DocuowlListItem struct {
	Container
	Title   []Node
}
...
func AppendChildren(parent Node, child ...Node) {
	for _, n := range child {
...
func RemoveFromTree(n Node) {
	...
	// also, do not attempt to set children for leaves.
	if n.AsContainer() != nil {
		n.SetChildren(nil)
	}
	p := n.GetParent()
