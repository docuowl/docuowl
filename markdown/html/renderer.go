...
type RendererOptions struct {
	...
	// Defaults (note content quote is not closed) to `  <meta name="GENERATOR" content="github.com/heyvito/docuowl/markdown markdown processor for Go`
	Generator string
}
...
func NewRenderer(opts RendererOptions) *Renderer {
	...
	if opts.Generator == "" {
		opts.Generator = `  <meta name="GENERATOR" content="github.com/heyvito/docuowl/markdown markdown processor for Go`
	}
...
func (r *Renderer) owlBoxEnter(w io.Writer, nodeData *ast.DocuowlBox) {
	r.CR(w)
	r.Outs(w, fmt.Sprintf(`<div class="box"><div class="title"><p>%s</p></div>`, nodeData.Title))
}

func (r *Renderer) owlBoxExit(w io.Writer) {
	r.Outs(w, "</div>")
}

func (r *Renderer) DocuowlBox(w io.Writer, node *ast.DocuowlBox, entering bool) {
	if entering {
		r.owlBoxEnter(w, node)
	} else {
		r.owlBoxExit(w)
	}
}
...
func (r *Renderer) owlListEnter(w io.Writer, nodeData *ast.DocuowlList) {
	listLevel++
	r.CR(w)
	if nodeData.HasTitle {
		r.Outs(w, fmt.Sprintf(`<h5>%s</h5>`, nodeData.Title))
	}

	if listLevel > 1 && !nodeData.HasTitle {
		r.Outs(w, `<details><summary><svg style="width: 10px;height: 10px;" height="16" viewBox="0 0 16 16" width="16" xmlns="http://www.w3.org/2000/svg"><path d="M9 7h6a1 1 0 0 1 0 2H9v6a1 1 0 0 1-2 0V9H1a1 1 0 1 1 0-2h6V1a1 1 0 1 1 2 0z" fill-rule="evenodd"></path></svg></summary>`)
	}
	r.Outs(w, `<ul class="owl-list">`)
}

func (r *Renderer) owlListExit(w io.Writer, nodeData *ast.DocuowlList) {
	listLevel--
	r.Outs(w, "</ul>")
	if listLevel > 1 && !nodeData.HasTitle {
		r.Outs(w, "</details>")
...
func (r *Renderer) DocuowlList(w io.Writer, node *ast.DocuowlList, entering bool) {
	if entering {
		r.owlListEnter(w, node)
	} else {
		r.owlListExit(w, node)
	}
}

func (r *Renderer) owlListItemEnter(w io.Writer, nodeData *ast.DocuowlListItem) {
	r.CR(w)

	r.Outs(w, `<li class="title">`)
	for _, t := range nodeData.Title {
		ast.WalkFunc(t, func(node ast.Node, entering bool) ast.WalkStatus {
			return r.RenderNode(w, node, entering)
		})
	}
	r.Outs(w, `</li>`)
	r.Outs(w, `<li class="content">`)
}

func (r *Renderer) owlListItemExit(w io.Writer) {
	r.Outs(w, `</li>`)
}

func (r *Renderer) DocuowlListItem(w io.Writer, node *ast.DocuowlListItem, entering bool) {
	if entering {
		r.owlListItemEnter(w, node)
	} else {
		r.owlListItemExit(w)
	}
}
...
func (r *Renderer) CodeBlock(w io.Writer, codeBlock *ast.CodeBlock) {
	...
	iterator, err := lex.Tokenise(nil, string(codeBlock.Literal))
	if err != nil {
		panic(err)
	}
...
func (r *Renderer) RenderNode(w io.Writer, node ast.Node, entering bool) ast.WalkStatus {
	...
	case *ast.BlockQuote:
		tag := TagWithAttributes("<blockquote", BlockAttrs(node)) + `<div class="blockquote-wrapper">`

		r.OutOneOfCr(w, entering, tag, "</div></blockquote>")
	case *ast.Aside:
	...
	case *ast.DocuowlBox:
		r.DocuowlBox(w, node, entering)
	case *ast.DocuowlList:
		r.DocuowlList(w, node, entering)
	case *ast.DocuowlListItem:
		r.DocuowlListItem(w, node, entering)
...
func (r *Renderer) RenderEmoji(w io.Writer, literal []byte) {
	emoji, ok := emojiAliases[string(literal)]
	if !ok {
		r.Outs(w, fmt.Sprintf(":%s:", literal))
	} else {
		r.Outs(w, emoji)
	}
}
