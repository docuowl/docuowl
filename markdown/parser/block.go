...
func (p *Parser) block(data []byte) {
	...
	for len(data) > 0 {
		...
		// prefixed Docuowl Block element:
		//
		// #! Title here
		if p.isPrefixDocuowlBlockElement(data) {
			data = data[p.prefixDocuowlBlockElement(data):]
		}

		// prefixed Docuowl List element:
		//
		// #- Title here
		if p.isPrefixDocuowlListElement(data) {
			data = data[p.prefixDocuowlListElement(data):]
		}
...
func (p *Parser) isPrefixDocuowlBlockElement(data []byte) bool {
	if p.extensions|Mmark == 0 {
		return false
	}
	if len(data) < 4 {
...
func (p *Parser) prefixDocuowlBlockElement(data []byte) int {
...
func (p *Parser) isPrefixDocuowlListElement(data []byte) bool {
	if p.extensions|Mmark == 0 {
		return false
	}
	if len(data) < 4 {
...
func (p *Parser) prefixDocuowlListElement(data []byte) int {
	i := skipChar(data, 2, ' ') // "#!" skipped
	end := skipUntilChar(data, i, '\n')
	skip := end
